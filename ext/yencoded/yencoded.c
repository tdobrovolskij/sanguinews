...
VALUE method_yencoded_data_yenc(VALUE self, VALUE data,VALUE length) {
  ...
  VALUE crc32;
  VALUE result;
  // table for crc32 value calculations
  ...
  while (restlen>0) {
    ...
    crc = crc32_tab[(crc ^ c) & 0xFF] ^ (crc >> 8); // crc32 calculation
  ...
  *output=0;			   //NULL termination is required
  ...
  crc = crc ^ ~0U;
  result=rb_ary_new();
  yencoded=rb_str_new2((char*)start);
  crc32=UINT2NUM(crc);
  rb_ary_push(result, yencoded);
  rb_ary_push(result, crc32);
  free(start);
  return result; 
}
